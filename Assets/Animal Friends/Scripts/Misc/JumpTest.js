...
function Start () {
	animation[upAnimation.name].layer = 1;
	animation[downAnimation.name].layer = 1;
	animation[landAnimation.name].layer = 0;
...
function Update () {
	...
	if(isGrounded){
		animation.Blend(upAnimation.name, 0, .05);
		animation.Blend(downAnimation.name, 0, .05);
		...
		animation.Blend(landAnimation.name, idleLandBlend, .05);
		animation.Blend(idleAnimation.name, 1 - idleLandBlend, .05);
	}
	else{
		isLanding = false;

		animation.Blend(upAnimation.name, upDownBlend, .05);
		animation.Blend(downAnimation.name, 1 - upDownBlend, .05);
